...
export class ResultsComponent implements OnInit {
  ...
  onBatchSelected(event) {
    ...
    if (this.selectedBatchString != '' && this.selectedBatchString != 'Select a batch') {
      this.examinerService.getData('batches/' + this.selectedBatchString + '/subjects').subscribe(
        (response) => {
  ...
  onSubjectSelected(event) {
    ...
    this.results = [];
    this.examinerService.getData('batches/' + this.selectedBatchString + '/results/' + selectedSubjectKey).subscribe(
      (response) => {
        if (response != undefined) {
          this.isResult = true;
